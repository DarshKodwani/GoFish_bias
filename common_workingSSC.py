...
class ParamRun:
    ...
    def __init__(self,fname) :
        ...
        self.dicov=np.zeros([self.npar_vary+self.npar_mbias,self.n_ell,
                               self.nbins_total,self.nbins_total])
        #Darsh end
        ...
        self.print_params()
        #Darsh test start
        #cosmo = ccl.Cosmology(Omega_c=0.27, Omega_b=0.045, h=0.67, A_s=2.1e-9, n_s=0.96, N_nu_rel=3.046, Omega_k=0.)
        #tr1_ids=[0,0,0]
        #tr2_ids=[0,0,0]
        #bn1_ids=[0,0,1]
        #bn2_ids=[0,1,1]
    ...
    def get_inverse_covariance(self,gst,invert=True) :
        ...
        if (self.icovar_ext_file is None) or (self.icovar_ext_file=='none') :
            ...
            if self.include_SSC:
                ...
                cosmo=ccl.Cosmology(Omega_c=0.27,Omega_b=0.045,h=0.67,A_s=2.1e-9,n_s=0.96,N_nu_rel=3.046,Omega_k=0.)
                ssc_cov=trc.SSC(cosmo,np.array(self.tracers),tr1_ids,tr2_ids,bn1_ids,bn2_ids,self.larr,self.d_larr,
                                self.fsky)
        ...
        if os.path.isfile(fname_save) :
            ...
            for i1 in np.arange(self.npar_vary+self.npar_mbias) :
                ...
                for i2 in np.arange(i1,self.npar_vary+self.npar_mbias) :
                    ...
                    self.fshr_cls[i1,i2]=np.dot(dclv[i2],np.dot(icovar,dclv[il]))
                    if i1!=i2 :
